...
请提供详细、准确且富有文化内涵的回答：`);
    ...
    try {
      ...
      const response = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('LLM响应超时')), 180000) // 增加到180秒（3分钟）超时
        )
      ]) as any;
  ...
  async testConnection(): Promise<boolean> {
    ...
    console.log('📋 使用的API Key:', config.deepseek.apiKey ? `${config.deepseek.apiKey.substring(0, 8)}...` : '未设置');
    ...
    try {
      ...
      const testMessage = await Promise.race([
        ...
        new Promise((_, reject) => 
          setTimeout(() => reject(new Error('测试超时')), 180000) // 增加到180秒（3分钟）超时
        )
      ]) as any;
    ...
    if (this.llm) {
      ...
      try {
        const simpleTest = await Promise.race([
          ...
          new Promise((_, reject) => 
            setTimeout(() => reject(new Error('简单测试超时')), 180000) // 增加到180秒（3分钟）超时
          )
        ]);
