...
export const ragApiMethods = {
  ...
  // 添加文本文档
  async addTextDocument(text: string, metadata?: Record<string, any>): Promise<any> {
    const response = await ragApi.post('/add-text', { text, metadata })
    return response.data
  },

  // 添加网页文档
  async addWebDocument(url: string, selector?: string): Promise<any> {
  ...
  async fullConnectionTest(): Promise<{ basic: boolean; rag: boolean }> {
